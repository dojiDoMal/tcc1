...
Após essa conversão, as coordenadas obtidas são multiplicadas pela matriz de projeção para definir como os vértices serão projetados na tela, os valores dessa matriz dependem se modo de projeção utilizado é em perspectiva ou ortográfico (Figura \ref{fig:frustum}) e são mostrados nas equações \ref{eq-persp} e \ref{eq-ortho} respectivamente. Os valores obtidos nessa operação são normalizados entre -1 e 1 para que seja feita a última operação de transformação, dessa vez as coordenadas são convertidas para o espeço de tela. A partir desse ponto elas continuam para o processo de rasterização da pipeline do OpenGL \cite{openglOnline}. 
	\begin{figure}[htp]
		\centering
